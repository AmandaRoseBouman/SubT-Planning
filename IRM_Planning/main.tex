...
\centering
    ...
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=1\columnwidth]{figures/diagram_v16.png}};
	    \begin{scope}[x={(image.south east)},y={(image.north west)}]
	    	...
	    	\node [font=\scriptsize,above left,align=right,black] at (0.9,0.08) {Sensor Data};
	    	...
	    	\node [font=\tiny,above left,align=right,black] at (0.189,0.58) {Local Coverage};
...
% [TODO: need this?] Note here that the policy search complexity grows exponentially with the number of nodes.
...
% [TODO: need this?] Once a local coverage behavior starts, the global coverage belief would change significantly, it is typically not much useful for GCP to plan beyond reaching a first target frontier node within a single planning episode.
%
...
% \sout{NOTE:
% NBV is using some of the resilience logic from POMCP MLP (e.g. replan distance, root-node updates).
...
% We compared our PLGRIM planner against two baseline algorithms built on next-best-view (competitive local coverage planner) and frontier-based schemes (competitive global coverage planner). 
\begin{enumerate}
    \item \ph{PLGRIM}
...
% \subsubsection{Environments for experiments} 
\begin{enumerate}
    ...
    % We include subway stations of varying sizes as structured urban environments for exploration. These environments are arguably simpler to explore than natural cave environments. They are easier to traverse due to the flat ground, have limited obstacles, and are fairly structured in the overall map. It is also easier to study the performance of exploration algorithms v/s the size of the environment.
    ...
    % This includes a simple cave world from DARPA SubT (or OSRF SubT) git repos. This includes a simple cave world with a straight path and fewer intersections. We also use a complicated maze-like cave environment to test the agent's ability to explore the map and then return to explored areas for further exploration in unexplored crossroads.
    ...
    % in California To verify the sim2real capabilities of our algorithm, we tested the robot in a lava tube (Valentine Cave) at the Lava Beds National Monuments in California, USA. We used a Spot robot from Boston Dynamics <TODO cite IROS Spot paper>, which had a traversable limit of <TODO> kms within the cave.
\end{enumerate}
...
\subfloat[traj of bot (yellow to purple in time)]{%
  \includegraphics[clip,width=0.3\columnwidth, angle=90]{figures/darpa_tv_test_figs/cave_path_overlay_lightbrown.png}%
}


\subfloat[exploration over time]{%
  \includegraphics[clip,width=0.4\columnwidth]{figures/darpa_tv_test_figs/distance_metrics.pdf}%
}
	\caption{Benchmark test for our method in a lava tube (valentine cave at Lava Beds National Monument)}
    \label{fig:darpatvtests}
\end{figure}
...
\centering
    \begin{tikzpicture}
	    ...
	    \end{scope}
	\end{tikzpicture}	
\caption{GCP} \label{fig:mlp_hardware_tests} 
\end{figure*}
