...
% This work proposes a scalable value learning framework, PLGRIM (Planning at Local-Global levels with Robust Information roadMaps), that bridges the gap between \textit{(i)} local, risk-aware resiliency and \textit{(ii)} global, reward seeking mission objectives.
...
%   \caption{Hierarchical Information RoadMaps (IRMs) for large-scale exploration in unknown environments.}
%   \label{fig:firstPage}
% \end{figure}
...
\centering
    ...
    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=1\columnwidth]{figures/hierarchy_labels_spot.png}};
	    ...
	    \end{scope}
	\end{tikzpicture}	
  \caption{Hierarchical Information RoadMaps (IRMs) generated during Au-Spot's autonomous exploration of a cave, Lava Beds National Monument, Tulelake, CA. } \label{fig:IRMs} 
\end{figure}
...
%   \caption{Illustration. Visualize the planned path in each level.}
...
% , there are three major factors for the performance in the global coverage: 1) replanning frequency (world representation update), 2) planning horizon at each planning episode, and 3) execution feasibility of the plan.

% With this formulation of the Receding Horizon Planning problem, let us now look at the considerations that determine performance algorithms and related challenges.
...
% 3. Actual execution of the plan (under uncertainty) --> planning consistency and resiliency
...
%  \item \textbf{Planning Horizon / Action Space Resolution}:

 
%  What we need:
%  - memory-efficient information to make decisions (critical and not all)
%  - multi-resolution action space
%  - efficent solver
 
%   The action space is composed of discrete robot motions. The action space resolution is defined according to the size of an action, or the separation between the robot's start and end locations during action execution. The planning horizon is defined as the number of actions planned into the future by the robot. 

%  The spatial and temporal planning horizon are defined with respect to the instantaneous world representation. The spatial horizon describes the size of a unit action
%  - Spatial: node vs. node
%  - Temporal: graph vs. lattice
...
%  - On Lattice: fixed temporal horizon, fixed spatial horizon
%  - On Graph: fixed spatial horizon, fixed temporal horizon

%  - reason about long distance motion plans, 
%  - small resolution actions --> reason over long temporal horizon 
%  - we need efficient planner (long horizon efficiently) and we need hierarchical action sets 
%  - how to specify action set resolution?
%  - given action set resolution, how to efficiently search over long horizon?
 
%  - Temporal Horizon: up to how long should we say we have to plan (mission duration?)
%  - How to solve it efficiently?
 
%  - Spatial Horizon: up to how many nodes should we plan, constrained by environment knowledge
%  - why do we need hierarchical structure?
 
%  Sec. 3 motivates 4.
 
%  Horizon is defined by environment knowledge. For lattice, we are dealing with static horizon dimension. For graph, we are limited to size of graph. Far ahead in time vs space. Temporal horizon may come into play when we think of task assignment, when you think of having limited resources and are operating in an environment with certain tasks that need to be accomplished, and each task ahs an associated reward, location, duration.
...
%  \begin{enumerate}
%   \item \label{en:issue2a} \textbf{Spatial Horizon}: One aspect of the planning horizon deals with the spatial effect of each action. This, in turn, determines the spatial extent of the multi-step look ahead. 
...
%   Temporal horizons represent time spans of a path, which are given by the robot’s motion model. 
  
% %   Temporal horizon--number of steps (size of the receding horizon window $T$, effectively the speed to reason over $T$ given planning time limit $\Delta t$)
%  \end{enumerate}

% - Robot dynamic constraints --> plan consistency 
%     - turning radius constraint --> minimize velocity changes
%     - smooth transitions between consecutive planning episodes
% - Sensor uncertainty + map delay due to sensor processing --> plan resiliency
%     - plan update to avoid hazardous areas 

% You need consistency across consecutive plans, so the robot does not make sudden changes in position or velocity when switching from one plan to another. Robot can replan if the robot has a position error. This resiliency to changes in pose or the surrounding map is a key challenge for the planner. This is even more important for safety-critical systems, such as a robot in a natural disaster. In this case, there are reactive low-level motion controllers that should be able to adjust its plan in response to nearby obstacles.

%  \item \textbf{Actuator uncertainty}: This refers to position uncertainty through motion. Challenges include consistency across plans and resiliency to a changing perception of the static environment. These may be opposing each other and need to be balanced.
%  Actual execution of the plan (under uncertainty) --> planning consistency and resiliency
%   (especially for safety-critical system, there are (possibly complex) additional layer of lower-level motion controllers; if new safety hazards appear during execution of the plan, this lower-level controller will adjust the plan to ensure the robot safety)
%  \begin{enumerate}
%   \item \label{en:issue3a} \textbf{Planning consistency}: You need consistency across consecutive plans, so the robot does not make sudden changes in position or velocity when switching from one plan to another.
%   \item \label{en:issue3b} \textbf{Planning resiliency}: Robot can replan if the robot has a position error. This resiliency to changes in pose or the surrounding map is a key challenge for the planner. This is even more important for safety-critical systems, such as a robot in a natural disaster. In this case, there are reactive low-level motion controllers that should be able to adjust its plan in response to nearby obstacles.
%  \end{enumerate}
% \end{enumerate}
...
%   \item 2.b. Longer temporal horizon --> efficient belief space planning (QMDP for GCP, POMCP for MLP)
%     (1-2 Hz replanning frequency; reason about hundreds of steps for GCP, tens of steps for MLP within 1 sec)
...
%   \item 3.b. Plan resiliency --> additional optimization factor when to depart
%     (high resiliency needed, especially in unstructured environments like natural caves)
% \end{itemize}
...
% resiliency and consistency over receding horizon planning at run time (our solution in Sec. 4: adaptive think-while-driving + hierarchical planning)
%
%
% OLD-3
...
%   \item Resiliency to stochastic motion and sensing
...
% \begin{figure}[t!]
%   \centering
%   \includegraphics[width=\columnwidth]{figures/diagram_v15.png}
% %   \includegraphics[width=0.5\columnwidth,trim={0 5.5cm 9.0cm 0},clip]{figures/diagram.png}
...
%   \STATE Add new frontier nodes $\{\hat{w}^g_f\} \in \hat{N}^g_{f^+}$ to $N^g_f$ with corresponding coverage probabilities $\{P(\hat{w}^g_{f,cov})\}$
  
  
  
% %   \STATE $\{\hat{w}^g_f\}$ $\gets$ FrontierManager(Riskmap, PoseGraph) 
%   \STATE $\{(\hat{w}^g_f, P(\hat{w}^g_{f,cov}))\}$ $\gets$ FrontierManager(Riskmap, PoseGraph) 
%   \STATE \hspace{4.5cm} $\triangleright$ \cite{keidar2012robot}
%   \STATE Add new frontier nodes $\{\hat{w}^g_f\}$ to $N^g_f$ with corresponding coverage probabilities $\{P(\hat{w}^g_{f,cov})\}$
% %   \FOR {each $\hat{w}^g_f \in \{\hat{w}^g_f\}$}
% %     \STATE Add a new frontier nodes $\hat{w}^g_f$ to $N^g_f$
% %     \STATE Add coverage probability $P(\hat{w}^g_{f,cov})$ to the properties of $\hat{w}^g_f$
% %   \ENDFOR
 
%   \STATE Frontier node candidates $\hat{N}^g_f$ $\gets$ FrontierManager(Riskmap, PoseGraph) 
%   \STATE \hspace{4.5cm} $\triangleright$ \cite{keidar2012robot}
%   \FOR {each $\hat{w}^g_f \in \{\hat{w}^g_f\}$}
%     \STATE Add a new frontier nodes $\hat{w}^g_f$ to $N^g_f$
%     \STATE Add coverage probability $P(\hat{w}^g_{f,cov})$ to the properties of $\hat{w}^g_f$
...
%   \FOR {each node $\hat{w}^g_i \in N^g_b \cup N^g_f$}
%     \STATE Compute the traversal distance and risk
%   \ENDFOR
...
%   \STATE $d(q', n_f) > \bar{d}_f$ for $\forall n_f \in N_f$
%   \STATE $T(q', n_b^{nn}) > \bar{\rho}_f$
%   \STATE Then add to $N^g_f$
  
%   \STATE $d(q, n_b) > \bar{d}_b$ for $\forall n_b \in N_b$, where $d(\cdot, \cdot)$ is a distance function (along a traversable manifold)
%   \STATE Connect to neighbor nodes 
  
%   \STATE Edges between breadcrumbs or between breadcrumbs-frontiers are generated if $d(n_1, n_2) < \bar{d}_e$ and $T(n_1, n_2) > \bar{\rho}_f$.

%   \STATE Frontier Removal
...
% Step 1: Simplification of pose graph. Using a node contraction algorithm based on edge distance (i.e., merge adjacent nodes until all the node distance becomes greater than X m). Assign traversability values to edges based on risk map
% Step 2: Attach aux nodes (e.g., frontiers) to the breadcrumb backbone based on traversability check
% Step 3: Populate additional edges based on traversability check. Run traversability checks for all pairs within distance threshold
...
% consider each belief space $b^\ell = (q, W^\ell) \in \mathbb{B}^\ell$ $b^g = (q, W^g) \in \mathbb{B}^g$.
% Then solve for the cascaded hierarchical POMDP problem for coverage in an unknown environment.
% --> informative path planning under motion/sensing uncertainty
...
% % $ H(W_{cov}) = -\sum_{\hat{w}^\ell_i \in N^\ell} [ P(\hat{w}^\ell_i) \log P(\hat{w}^\ell_i) + (1-P(\hat{w}^\ell_i)) \log (1-\hat{w}^\ell_i)) ] $
% % $H(W^\ell_{cov}) = \sum_{\hat{w}^\ell_i \in N^\ell} [ - P(\hat{w}^\ell_i) \log P(\hat{w}^\ell_i) - (1-P(\hat{w}^\ell_i)) \log (1-\hat{w}^\ell_i)) ]$
% % $H(W^\ell_{cov}) = \sum_{\hat{w}^\ell_{i} \in N^\ell} [ - P(\hat{w}^\ell_{i,cov}) \log P(\hat{w}^\ell_{i,cov}) - (1-P(\hat{w}^\ell_{i,cov})) \log (1-\hat{w}^\ell_{i,cov})) ]$
% $H(W^\ell_{cov}) = \sum_{i}^{|N^\ell|} [ - P(\hat{w}^\ell_{i,cov}) \log P(\hat{w}^\ell_{i,cov}) - (1-P(\hat{w}^\ell_{i,cov})) \log (1-\hat{w}^\ell_{i,cov})) ]$
% $H(W^\ell_{cov}) = \sum_{i}^{|N^\ell|} [ - P(\hat{w}^\ell_{i,cov}) \log P(\hat{w}^\ell_{i,cov}) - (1-P(\hat{w}^\ell_{i,cov})) \log (1-P(\hat{w}^\ell_{i,cov})) ]$
...
% --> Need very long horizon planning
...
% Note here that the policy search complexity grows exponentially with the number of nodes.
% First, the environment size can be very large (> 1~km), so that the planning horizon (the number of actions) for GCP can be more than hundreds.
% First, the environment size can be very large (> 1~km) so that more than hundreds of nodes can be added to the Global IRM.
% so that GCP should reason over more than hundreds of nodes on the Global IRM despite its sparse structure, where the search space complexity grows exponentially.
% First, the environment size can be very large (> 1~km) so that GCP should reason over more than hundreds of nodes on the Global IRM despite its sparse structure, where the search space complexity grows exponentially.
% the POMDP solving to be challenging. (to plan in real-time)
%
%
\phdone{GCP Problem Characteristics--Frontiers as terminal nodes}
% --> frontier nodes as terminal nodes of horizon
...
% Once a local coverage behavior starts, the global coverage belief would change significantly, it is typically not much useful for GCP to plan beyond reaching a first target frontier node within a single planning episode.
%
% considering GCP is for a guidance role
% lots of changes will be made to $\hat{W}_{cov}$ once LCP starts to see uncovered local area again
%
% % so, GCP seeking the maximum reward before making any change to the coverage state (pure exploitation)
%
% planning horizon up to a frontier node
% (regard frontier nodes as terminal nodes for this very-long discounted horizon planning)
%
%
...
% If we regard the frontier nodes as terminal nodes in the belief space, we do not need to consider any change of the world coverage state before the termination, which means we can omit $W$ from our state space for GCP.
...
%
% frontier nodes are terminal nodes,
% no changes can be made to the coverage state,
% what we want to do is to reason about the best one and only frontier node to reach and quit
%
% no consideration about coverage state changes in the planning problem,
% so, we can drop $W$ from our state space
% after the first step from the current robot to any of the node on the Global IRM (under motion uncertainty), everything else is approximated as an MDP
% -> then now we can adopt QMDP
...
% \begin{figure}[t!]
%   \centering
%   % \includegraphics[width=0.5\columnwidth]{figures/LCP-Rollout-Example.png}
%   \includegraphics[width=0.5\columnwidth]{example-image}
%   \caption{[TODO] Illustration (toy example for rollout) of POMCP solver for Lattice-Level Planning (LCP).}
...
% [TODO: soft optimization version?]
% $R^\ell(s^\ell, a^\ell) = \lambda [k_I I(W^\ell_{cov}, z) - k_C C(W^\ell_{occ}, q^\ell, a^\ell))] + (1 - \lambda) [ k_g S(\hat{w}^g_f, \cdot) ]$,

% Optimization mode, given $\Theta^\ell$ from GCP, LCP does ...
...
% % \STATE
% \STATE \textbf{input: }Beliefs $b^g$, $b^l$ of states $s=(G, Q)$
% \STATE \textbf{input: }World configuration $W$
...
% RHP given past policy
% balance between consistency and resiliency
% joint optimization
%
% how to simplify it to solve easily
% simplified first part of policy to reuse previous policy
...
% % One main point we want to show in MLP is the consistency + resiliency (which may conflict to each other). Consistency is needed in general to make a smoother/full-speed motion of the robot (like 1->2). Resiliency is needed when some obstacle pops up and path should be adaptively changed (like 2->3).
...
%
% Note, however, that a consistent path for the robot reduces it’s ability to react quickly to changes in the perceived map. This resiliency could be required, for example, when the sensor readings from far away show a location as free, but when we come closer and collect more readings, it turns out to be an obstacle at close distance. Hence, we need to combine and weigh these aspects of the planner to make it both consistent and resilient in it’s planning.


% Due to the nature of unknown space exploration with a limited field of view, the agent \ncomment{decision making} only has access to a part of the environment $W$ at anytime $t$. Hence, to fulfil its goal of exploration, we formulate the planning problem such that the agent gets reward from information gain (e.g. uncovering new regions).
%
% We design this information gain from observations the robot has made. These observations include sensor information such as accelerometers, motor encoders, \ncomment{images?} etc. The observations are gathered as the robot executes motion plans in the environment and forms a graphical representation with information stored mapped to the pose of the robot, known as the pose graph.
%
% Note that these plan executions are dependent on the robot state at the \textit{execution} layer, using Model Predictive Control. The heading and velocity of the robot at the end of a path can significantly affect the execution of the next path. Thus, during planning we have to consider the previous path the robot has taken as well, to ensure path consistency during exploration. Such a consistent path will increase the information gain per unit time.
...
% The  consists of LiDAR range sensors which return 3D point cloud data. The entire autonomy stack runs in real-time on an Intel Core i7 processor with 32 GB of RAM.
% We test in two environments - a maze-like cave of size $400$m$ \times 100$m with complex terrain and topology, including narrow passages, dead-ends, open-spaces, and branches with fluctuating curvature, and a large subway station $120$m $\times 40$m with open spaces as well as narrow passages.

% Summary on baselines

% NBV (using Local IRM): competent local coverage planner, but no consideration about risk-awareness or information-gathering within the field-of-view

% HFE (using Global IRM): competent global coverage planner with local coverage scheme as well, but brittle mission performance due to the sparse world representation


% CONTENTS
% - baseline
%     : may-demo (Frontier-based exploration)
%     : GLP-MLP
...
% \sout{NOTE:
% NBV is using some of the resiliency logic from POMCP MLP (e.g. replan distance, root-node updates).
% Do some random walk when get stuck.\\
% NBV runs a deterministic path planner at the lower level to get a path to each candidate view points, so it cannot consider risk-aware or information-gathering paths that are not of the shortest distance to the view points.\\
% May (HFE) vs. Ours: At a high-level (using the same global IRM representation), their covered area has very similar trend as long as the mission progresses. Then the key is who can continue the mission without getting stuck.}
...
% We compared our PLGRIM planner against two baseline algorithms built on next-best-view (competent local coverage planner) and frontier-based schemes (competent global coverage planner). 
% \begin{enumerate}
%     % \item \ph{PLGRIM}
%     % The Global IRM, a sparse bidrectional graph structure spanning the known environment, was defined to have a \textit{breadcrumb} resolution of $5~m$ and a \textit{frontier} detection range of $8~m$. The Local IRM, a rolling grid structure centered at the robot's current position, was defined to be 20x20 with a $1~m$ resolution.
...
%     \item \ph{Local IRM-based Next-Best-View (NBV)} 
%     % NBV is a one-step lookahead algorithm that estimates the gain of information associated with moving to a position within the robot's field-of-view $F$ \cite{bircher2016receding}. NBV was implemented on the Local IRM by computing the traversal distance and information gain associated with each reachable node within $F$. The node that maximizes an objective function, which considers information gain and traversal distance, is selected as the next goal.
    
%     Nodes in the robot's field-of-view are myopically selected according to an objective function that weights infomration gain against action cost.  
    
%     \item \ph{Hierarchical Frontier-based Exploration (HFE)}
%     % This is a depth-first search along the global IRM to find the next frontier node to explore. As it operates solely on the global IRM, it does not have a hierarchical approach like PLGRIM.
%     % Frontiers in the global IRM graph encode a gain in spatial information for one lookahead step. 
%     Frontiers in the Global IRM are myopically selected according to a local or global policy that aim to maximize information gain and minimize action cost by using high- and low- fidelity information, accordingly. 
    
%     % Frontiers that satisfy certain traversability and field-of-view conditions serve as the local action space, which is employed by the local policy. If the local action space is empty, then a global policy determines the next best frontier.  
    
%     % then  The local policy selects a frontier according an objective function that seeks to minimize path costs. If the local action space is empty, then a global policy determines the next best frontier according to a objective function that weighs information gain against action cost.
% \end{enumerate}
...
% The proposed PLGRIM and baseline planners were tested in simulated subway stations and caves, as well as in a real-world lava tube cave. 

% \ph{Simulated Subway Station} large interconnected, pologynal rooms with level-floors, which are free of obstacles.

% \ph{Simulated Caves} maze-like environments with complex terrain (e.g. rocks, steep slopes) and topology, including narrow passages, dead-ends, open-spaces, and branches with fluctuating curvature.  

% \ph{Lava Tube Cave} located in Lava Beds National Monument, Tulelake, CA. The cave consists of a main lava tube and a series of smaller, auxiliary tubes branching off. The floor is composed of undulating smooth and ropy masses of cooled lava. Some areas of the floor are covered by large boulders, due to ceiling breakdown. 
...
\begin{figure}[t!]
    \centering
     	\subfloat[Subway 1x]{\includegraphics[width=0.392\columnwidth]{IRM_Planning/figures/areacov_plots/subway1x.pdf}}
    	\subfloat[Subway 2x]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/subway2x.pdf}}	
    	\subfloat[Subway 3x]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/subway3x.pdf}}	
    \caption{Exploration performance of PLGRIM planner and baseline in simulated subway environments of increasing size.}
    \label{fig:subway_plot}
\end{figure}
...
\begin{figure}[t!]
    \centering
      \subfloat[][Simulated \\Cave]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_simple_cave_legend.pdf}}
      \subfloat[][Simulated \\Maze]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_cave_cropped.pdf}}
    	\subfloat[Lava Tube]{%
        \includegraphics[clip,width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/lava_tube.pdf}%
        }
    \caption{Exploration by PLGRIM and baseline methods in simulated and real-world cave environments. Each algorithm was tested twice in the simulated cave and maze.}
    \label{fig:cave_plot}
\end{figure}
...
\begin{figure*}[t!]
    \centering
    	...
    	\subfloat[Subway 2x]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/subway2x.pdf}}	
    	\subfloat[Subway 3x]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/subway3x.pdf}}	
      \subfloat[][Simulated \\Cave]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_simple_cave.pdf}}
      \subfloat[][Simulated \\Maze]{\includegraphics[width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_cave_cropped.pdf}}
    	\subfloat[Lava Tube]{%
        \includegraphics[clip,width=0.31\columnwidth]{IRM_Planning/figures/areacov_plots/lava_tube.pdf}%
        }
...
%      	\includegraphics[width=0.5\columnwidth]{IRM_Planning/figures/together.png}
%     %  	}
%     % %  	\quad
%     % 	\subfloat[Simulated \\Maze]{\includegraphics[width=0.3\columnwidth, angle=90]{IRM_Planning/figures/maze.png}}
%     \caption{Maps of the simulated cave (top) and maze (bottom)}
...
% Note that our algorithm far exceeds the area covered compared to other algorithms in the unstructured case.

% There are many sharp corners in the environment, where the frontier-based exploration approaches may fail to detect the frontiers and abandon exploring beyond the corner.
...
% \begin{figure}
%      \centering
%  	\subfloat[traj of bot (from yellow to purple in time)]{\includegraphics[width=0.45\columnwidth]{IRM_Planning/figures/darpa_tv_test_figs/cave_path_overlay_lightbrown.png}}
%  	\quad
% 	\subfloat[exploration over time]{\includegraphics[width=0.45\columnwidth]{IRM_Planning/figures/darpa_tv_test_figs/distance_metrics.pdf}}\\	
% 	\caption{HORIZONTAL - side by side}
% % 	Benchmark test for our method in a lava tube (valentine cave at Lava Beds National Monument)
%     \label{fig:darpatvtests}
% \end{figure}
...
%     \includegraphics[width=\textwidth]{figures/MLP_screenshot.png}
%     \caption{Flower one.}
%   \end{minipage}
%   \hfill
%   \begin{minipage}[b]{0.4\textwidth}
%     \includegraphics[width=\textwidth]{figures/GLP_screenshot.png}
%     \caption{Flower two.}
%   \end{minipage}
% \end{figure}
...
% 	    	% Annotations [Time]
% 	    	\node [font=\scriptsize,above left,align=right,white] at (0.245,0.58) {05:08};
% 	    	\node [font=\scriptsize,above left,align=right,white] at (0.495,0.58) {05:20};
% 	    	\node [font=\scriptsize,above left,align=right,white] at (0.742,0.58) {05:24};
% 	    	\node [font=\scriptsize,above left,align=right,white] at (.992,0.58) {06:24};
...
% 	    \end{scope}
% 	\end{tikzpicture}	
% \caption{GCP plans path (blue) along Global IRM to frontier after local area is fully covered (snapshot A). Robot explores area around frontier (snapshot B), and then explores neighboring frontier at opening of side channel. LCP plans path (green) into channel (snapshot C). Later, after all local area has been explored, GCP guides robot back towards mouth of cave (snapshot D).} \label{fig:mlp_hardware_tests} 
% \end{figure*}

% \begin{figure}[t!]
%     \centering
%      	\subfloat[Subway 1x]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/subway1x.pdf}}
%     	\subfloat[Subway 2x]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/subway2x.pdf}}	
%     	\subfloat[Subway 3x]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/subway3x.pdf}}	
%     \caption{Exploration performance for increasing size of the world $W$}
%     \label{fig:subway_plot}
% \end{figure}
...
% \begin{figure}
%     \centering
%      	\subfloat[Simple cave]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_simple_cave.pdf}}
%     	\subfloat[Cave]{\includegraphics[width=0.3\columnwidth]{IRM_Planning/figures/areacov_plots/comparison_cave_cropped.pdf}}
%     	\subfloat[Lava Tube]{%
%         \includegraphics[clip,width=0.3\columnwidth]{figures/darpa_tv_test_figs/distance_metrics.pdf}%
%         }
%     \caption{Exploration by PLGRIM and baseline methods in simulated cave environments. We conducted 2 runs per algorithm per environment.}
%     \label{fig:cave_plot}
% \end{figure}

\begin{figure}
     \centering
% \subfloat[traj of bot (yellow to purple in time)]{%
  \includegraphics[width=0.45\columnwidth,angle=90, scale=0.7
  ]{figures/darpa_tv_test_figs/cave_path_overlay_lightbrown.png}%
% }
% 
% 
% \subfloat[exploration over time]{%
%   \includegraphics[clip,width=0.45\columnwidth]{figures/darpa_tv_test_figs/distance_metrics.pdf}%
% }
...
% \ph{Table?} Table~\ref{tab:all-expt-results} shows the exploration ability of each algorithm in each environment. Each algorithm is given a fixed time to explore the environment (30 min for structured subway environments, 40 min for cave environments). For the hardware experiments, we did not test every algorithm due to safety concerns about the robots movement.
% \begin{table}[]
% \begin{tabular}{c|c|c|c}
% \textbf{Environment}                                              & \textbf{NBV} & \textbf{HFE} & \textbf{PLGRIM (Ours)} \\ \hline
% Simple Cave                                                       &              &                   &                        \\
% Cave                                                              &              &                   &                        \\
% Subway 1x                                                         &              &                   &                        \\
% Subway 2x                                                         &              &                   &                        \\
% Subway 3x                                                         &              &                   &                        \\
% \begin{tabular}[c]{@{}c@{}}Lava tube\\(real world)\end{tabular}   &              &                   &                       
% \end{tabular}
% \caption{Area covered by algorithms for different environments}
% \label{tab:all-expt-results}
% \end{table}
...
% We tested our planner on the robot in the Valentine cave at Lava Beds National Monument. Fig.~\ref{fig:mlp_hardware_tests} shows the local IRM used by the Local Coverage Planner (LCP), and Fig.~\ref{fig:glp_hardware_tests} shows the global IRM used by the Global Coverage Planner (GCP). Fig.~\ref{fig:lava_tube_traj} shows the trajectory overlaid onto the cave image, and Fig.~\ref{fig:glp_hardware_tests} (c) shows the area covered over time. These runs validate the simulation-to-real (sim2real) transfer of our planner on the real robot in a real environment, and a video of the run is in the supplementary material.
...
%   : curse of history
%     . receding horizon planning
%     . resiliency and consistency for unknown environment exploration with MPC
% - benefit
...
Future work includes learning-based methods for graph expansion information gain estimation, and a richer incorporation of risk and time into the graph planning framework. % planning with a 3D traversabilty map??
...
% % Connection between GCP and LCP
...
% % Output of GCP is $\theta^\ell_t$, which gives a goal position to the robot executive (mobility service) to move the robot to an unexplored area of the map.
...
% % Output of GCP is $a^{*l}_{t:T}$, which is a series of robot poses to reach, given to the robot executive.
...
% This information gain is a metric we have designed \ncomment{learned?} from observations the robot has made. These observations include sensor information such as accelerometers, motor encoders, \ncomment{images?} etc. The observations are gathered as the robot executes motion plans in the environment and forms a graphical representation with information stored mapped to the pose of the robot, known as the pose graph. \gautam{exclude this?}
