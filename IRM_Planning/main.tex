...
\subsection{Basic POMDP Formulation}

\ph{Structure} A POMDP is described as a tuple $\langle \mathbb{S}, \mathbb{A}, \mathbb{Z}, T, O, R \rangle$, where $\mathbb{S}$ is the set of states of the world, $\mathbb{A}$ is the set of actions, and $\mathbb{Z}$ is the set of observations sensed by the agent of the world. The state $s = (M, q) \in \mathbb{S}$ consists of the static environment map $M$ and the robot pose $q$. At every time step, the agent performs an action $a \in \mathbb{A}$ and receives an observation $z \in \mathbb{Z}$ resulting from the agent's perceptual interaction with the environment. The motion model $T(s, a, s') = P(s'\,|\,s, a)$ defines the probability of being at state $s'$ after taking an action $a$ in state $s$. The observation model $O(s, a, z) = P(z\,|\,s, a)$ is the probability of receiving observation $z$ after taking action $a$ in state $s$. Finally, the reward function $R(s,a)$ returns the expected utility for executing action $a$ in state $s$.

\ph{Belief States} Since the state of the world is not fully observable, the agent maintains a belief state $b_t\in \mathbb{B}$ defined as the posterior distribution over all possible states conditioned on past actions and observations at time $t$. The belief over state $s$ is $b_{t} = P(s \,|\, a_{0:t-1}, z_{1:t})$. Our belief state is a two-element tuple $b=p(M,q)$ consisting of the partially observed map $M$ and the robot pose $q$. After the agent executes action $a_{t-1}$ and receives observation $z_t$, the belief state can be updated according to Bayes algorithm:
\begin{align}
...
\ph{Optimal Policy} A policy $\pi : \mathbb{B} \rightarrow \mathbb{A}$ maps each belief state $b$ to an action $a$. Our objective is find the policy that minimizes the time required to completely cover an unknown environment. To compute the optimal policy, we rely on the one-step reward function $R(b_t, a_t)$ that encodes the desired behavior of the robot: 
\begin{align}
...
The optimal policy maximizes the expected discounted sum of future reward:
\begin{align}
...
Receding horizon planning (interleaving planning and execution)
...

\ph{Advantage of using an abstract representation}
(compared to concrete? representation)
Flexible and representation-rich format?
System-agnostic
Scalable
General cost functions (incorporate multiple objectives) -- IRM can encode multi-dimensional information feature vector (lidar, artifact1, artifact2, etc.)
...
coverage application -> spatial information + metadata (e.g. co2 sensor reading etc)
so it extends to multi-dimensional information feature vectors


\ph{Key/common components (Highlight)}

Abstract state representation for coverage

Generative model construction from prior



\ph{Environment Representation}
$G = (V, E)$
...
\ph{State} The system (robot-world) state is a 2-tuple $s = (W, Q) \in \mathbb{S}$ consisting of the current abstract world representation $W$ and the node $Q$ representing the robot state. Each node $v \in V$ stores its occupancy and coverage state.

$W = (W_{occ}, W_{cov})$

$W_{occ}$ information/representation about occupancy (geometry) of the parts of the environment
...
\ph{Belief State} $b \in \mathbb{B} = p(s) = p(W, Q)$
...
\ph{Observation} The observation $z \in \mathbb{Z}$ received by the robot are the nodes directly connected to node $q_t$. The observation model is defined as $O: Q \times W \rightarrow \mathbb{Z}$. % where $S$ is one edge length.

$\hat{O}(s, a, z; W_{occ,0}, W_{cov,0})$

\ph{Reward} $R: \mathbb{Q} \times \mathbb{W} \times \mathbb{A} \rightarrow \mathbb{R}$ \\
...
The proposed hierarchical planner decomposes the problem into tractable subproblems by introducing a spatial and temporal discretization which restrict the behavior of the robot to a smaller, information-efficient search space. The hierarchical planner has two layers with different map representations. The higher layer, termed \emph{graph-level planner}, employs a sparse graph structure of dynamic size which spans the entirety of the known environment. The lower layer, termed \emph{lattice-level planner}, employs a dense grid structure of fixed dimensions which moves with the robot. 
...
\ph{Environment Representation} We employ a sparse bidirectional graph structure $G = (V, E)$ that captures the connectivity of the free space in the environment. We refer to this graph as the information roadmap (IRM) as its nodes $V$ and edges $E$ are enriched with environmental information. Each node $v \in V$ has attached to it a feature vector containing the probability $p(occ(v))$ that the node is occupied and the probability $p(cov(v))$ that the node was previously perceived by the robot's sensor suite $S$, constrained by viewing angle and distance. Each edge $e_{ij} \in E$ has attached to it a feature vector containing the probability $p(\rho(v_i,v_j)$) of traversal between the connected nodes $v_i$ and $v_j$.
