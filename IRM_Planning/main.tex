...
% \gautam{add traversability, occupancy info?}
...
The rest of the system architecture is as follows. As the robot moves with commands from the executive, it's sensors are used to update the costmap and pose-graph data (is this explained?). These are used to update the estimates of the generative models $\hat{\mathcal{G}}^g$ and $\hat{\mathcal{G}}^l$ at the global and local levels respectively. 
% \gautam{add traversability, occupancy info?}

* TODO: Add more details from system overview

% * LLP explores locally; sends move commands to executive. If it can explore no further, it then passes through the commands from GLP to exective. These commands take the robot to a new location in the world, where it can resume local exploration.
