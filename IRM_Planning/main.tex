...
\ph{Key characteristics} We present an abstract coverage planner formulated as a POMDP solver...
Maintains belieWorld representation

\ph{Advantage (POMDP)}
Longer-horizon planning
General cost functions (incorporate multiple objectives)

\ph{Advantage (Abstract representation)}
System-agnostic
Scalable

\ph{Key/common components}
...
\ph{Environment Representation} We employ a rolling, fixed-sized lattice structure which is centered at the robot's current position. Like the graph structure, each node $v \in V$ has attached to it a feature vector containing the probability of occupancy $p(occ(v))$ and the probability $p(cov(v))$ that the node was previously perceived by the robot's sensor suite $S$, constrained by viewing angle and distance. The lattice structure has additional edge information-- each edge $e \in E$ has attached to it a feature vector containing the probability $p(\rho(v_i,v_j)$) of traversal between connected nodes $v_i$ and $v_j$.
...
\ph{Observation} The observation $z \in \mathbb{Z}$ are nodes immediately visible from the robot's position subject to ray tracing. The observation model is defined as $O: \mathbb{Q} \times V^{occu} \times V^{cov} \rightarrow \mathbb{Z}$ where $S$ is pre-specified sensor range.
