...
\ph{RL-based approaches}
Intrinsic Curiosity Module \cite{pathak_icm}: you can formulate this with 'surprise', the more difference between your predicted state and actual state, the more the surprise factor. This can fail in certain situations. The most popular experiment on this was from \cite{burda2018study}, where an agent is dropped in an env where it can move but also change channels on a TV (if it is near the TV). The agent would just be drawn to the TV because changing the channels constantly gave it a new visual reward and it wouldn't explore the rest of the map. This was known as a 'couch-potato' behaviour. Thus, the agent was curious, in a sense, but not productive in exploring the map.

Episodic Curiosity through Reachability \cite{ECR2018}: There are agents that mark parts of the map as 'far away' in an effort to deincentivize the single-step jumps such as changing a TV channel. These agents go through the environment and are only rewarded for observations that are relatively far away from the current observation. While this is a potential solution for the couch-potato effect, we now need to track potential landmarks where the agent has been to keep track of which parts are already explored. But there is no such memory for the agent. Instead the environment has a storage of visited landmarks and gives the agent an augmented reward based on proximity to currently explored landmarks.

There's also something on random network distillation \cite{rnd} to help explore.
...
\section{Exploration-Coverage Planning Problem}

\ph{Intro} Exploration and coverage of a partially-revealed environment can be modeled as a Partially Observable Markov Decision Process (POMDP) where the agent maintains a belief about its environment and plans a sequence of actions with the ultimate goal of minimizing environment belief uncertainty. By predicting how future sensor information and vehicle control will affect this belief, the agent can make near-optimal decisions that are robust to environment uncertainty. Our objective is to find the policy that minimizes the time required to completely cover a large, unknown environment.

\ph{Traditional Map Representation} The belief state for this exploration-coverage POMDP is $b = p(M, x)$ where $M$ is the map state and $x$ is the robot state. Traditionally, $M$ is defined as an occupancy map composed of grid cells:
\begin{align}
    m = \{\textbf{m}_i\}
\end{align}
where $\mathbf{m}_i$ is a random variable representing the state (i.e. occupancy, coverage) of the $i$-th cell. The goal of belief grid mapping is to compute the posterior probability of the state of the whole map $M$, given the observations $z_{1:t}$ and robot trajectory $x_{1:t}$ up to time $t$:
\begin{align}
  p(M | z_{1:t}, x_{1:t}),
\end{align}
Unfortunately, it is usually intractable to compute directly from $M$ due to its complexity of $2^{|M|}$. So, the belief grid map is often assumed to be a Markov random field of order 0 in which the belief probability of each grid cell is independent \cite{TBF05,elfes1990stochastic}.
With the cell independence assumption, $p(M | z_{1:t}, x_{1:t})$ can be determined from the individual cell state probability, $p(\mathbf{m}_i | z_{1:t}, x_{1:t})$.

\ph{Computational Challenges} The problem in Eq. (\ref{eq:optimal_policy}) is intractable because of 1) the high dimension of the belief state $b$, particularly with respect to the belief over the map $M$, and 2) the large timescale $L$, since the complexity of the search space grows exponentially with time. To obtain a tractable solution, we discretize both the belief space $b=p(M,x)$ as well as the planning horizon. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Hierarchical Coverage Planner POMDP Framework}
...
\ph{Key characteristics} We present an abstract coverage planner formulated as a variant of the POMCP solver... 
...
%   \hat{R}(s, a) = F(\text{info gain}(W_{cov}, z), \, \text{action penalty}(W_{occ}, a)): \, \mathbb{Q} \times  \mathbb{W} \times \mathbb{A} \rightarrow \mathbb{R} \\
    \hat{R}(s, a) = \mathcal{F}\Big[\, \textit{Info\_Gain}(W_{cov}, z), \; \textit{Cost}(W_{occ}, a) \, \Big]: \, \mathbb{Q} \times  W \times \mathbb{A} \rightarrow \mathbb{R} 
...
The proposed hierarchical planner decomposes the problem into tractable subproblems by introducing a spatial and temporal discretization which restricts the behavior of the robot to a smaller, information-efficient search space. The hierarchical planner has two layers with different environment representations. The higher layer, termed \emph{graph-level planner}, employs a sparse graph structure of dynamic size which spans the entirety of the known environment. The lower layer, termed \emph{lattice-level planner}, employs a dense grid structure of fixed dimensions which moves with the agent. 

\ph{Overall policy} We define an overall policy $\pi \in \Pi$ generated by combining the high-level policy $\pi^g$ and the low-level policy $\pi^l$:
\begin{align}
    % \pi(b;v):\mathbb{B}\times V\rightarrow \mathbb{A}
    \pi(b):\mathbb{B}\rightarrow \mathbb{A} \\
    \pi(b;v) = \pi^l(b; \pi^g(v)) \\
    \pi(b) = \pi^l(b^l; \, \Theta^l) = \pi^l(b^l; \, \pi^g(b^g))
\end{align}

We introduce a nested, hierarchical formulation


Consider two hierarchical levels in this description.

For lowest level planner, the action set is concrete controller to move the robot

For any other higher level planners, the action set is a hyper-parameter to its lower level planner that encodes the higher level action specification...?

$\mathbb{A}^g = \Theta^l$

Policy decomposition
$\pi^l: \mathbb{B}^l \times \Theta^l \to \mathbb{A}^l$
where $\Theta^l$ is controller specific parameter 

$\pi^g: \mathbb{B}^g \to \Theta^l$
where $\Theta^l$ is parameter for lower-level policy.

$\pi(b) = \pi^l(b^l; \, \Theta^l) = \pi^l(b^l; \, \pi^g(b^g))$

$r^l(b^l, a^l) = r^l(b^l, \pi^l(b^l; \Theta^l) = r^l(b^l, \pi^l(b^l; \pi^g(b^g)))$

Unified policy is a composite (??) of higher and lower level policies
\begin{align}
...
\begin{align}
  \pi^{l*}(b) &= \arg\max_\pi \, \mathbb{E} \left[ \sum_{t=0}^{L} \gamma^t r^l(b^l_t, \pi^l(b^l_t; \Theta^l) \right]
  \label{eq:optimal_policy}
\end{align}
where $\Theta^l$ is given, 
which means this policy is a myopic/constrained/... but concrete policy.

\begin{align}
  \pi^{g*}(b) &= \arg\max_\pi \, \mathbb{E} \left[ \sum_{t=0}^{L} \gamma^t r^g(b^g_t, \pi^g(b^g_t) \right]
  \label{eq:optimal_policy}
\end{align}
where its action set $\Theta^l$ is not lowest level controller, so it cannot specify concrete actions of the robot, 
which mean this policy is approximate but global/non-myopic/... policy.


Given the current node, the high-level policy chooses the next local controller, and the low-level policy generates the corresponding local control signal at belief $b(s)$. This overall policy is stated as follows:
\begin{align}
    \pi(b;v) = \pi^l(b; \pi^g(v))
\end{align}
...
(nested, hierarchical formulation)
graph-level policy
-- lattice-level policy
...
\subsection{Proposed Framework (GOOD NAME??)} 

\subsubsection{Graph-Level Planner} 
...
\subsubsection{Lattice-Level Planner} 

\ph{Environment Representation} We employ a rolling, fixed-sized lattice structure $G^\ell = (V^\ell, E^\ell)$ which is centered at the robot's current position. Like the graph structure, each node $v^\ell \in V^\ell$ has attached to it a feature vector containing the node position, probability of occupancy $p_{occ}(v^\ell)$, and probability $p_{cov}(v^\ell)$ that the node has been sensed by the robot. The lattice structure also includes edge information -- each edge $e^\ell \in E^\ell$ has attached to it a feature vector containing the probability of traversal between connected nodes $v_i^\ell$ and $v_j^\ell$:
\begin{align}
    \rho_{ij}^\ell = \mathcal{F} \big(v_i^\ell, \, v_j^\ell; \, p(W_{occ})\big)
\end{align}
where $W_{occ}$ encodes the 3D volumetric occupancy information acquired from raw sensor data. 

% The traversal probability $\rho_{ij}^\ell$ is computed from $W_{occ}$, which encodes the 3D volumetric occupancy information acquired from raw sensor data.
% where $\rho^\ell$ is traversal risk and $p_{occ}(\cdot)$ is the probability of occupancy.

\ph{State} Two-tuple $s^\ell=(G^\ell, Q^\ell)  \in \mathbb{S}$ consisting of the current lattice state $G^\ell$ and robot state $Q^\ell = (q, \dot{q})$ where $q$ is the node closest to the robot's position and $\dot{q}$ is the change in nodal position during one time-step.

\ph{Observation} The observation $z \in \mathbb{Z}$ are nodes immediately visible from the robot's position subject to ray tracing. The observation model is defined as $O: q \times V^{occ} \times V^{cov} \rightarrow \mathbb{Z}$ where $S$ is pre-specified sensor range.


$w_{cov}^l = O(...) \in \mathbb{Z}^l$


% $w_{cov}^l = \hat{O}() \in \mathbb{Z}^l$ \\
% $N = \{v \in V \, | \, d(v,Q) < r\}$ where $r$ is sensor range



\ph{Action} Defined in "Abstract Planner"

\ph{Transition Model} We define the output of the lattice policy $\pi^\ell(v_i^\ell)$ to be a neighboring node $v_j^\ell$, which is connected by edge $e_{ij}^\ell$. We assume that $\pi^\ell(v_i^\ell)$ induces a transition from nodes $v_i^\ell$ to $v_j^\ell$ equal to the probability of traversal:
\begin{align}
    T^{g} (v_i^\ell, \pi^\ell(v_i^\ell), v_j^\ell) = \rho_{ij}^\ell
\end{align}

\ph{Reward Function} Action cost: rotation penalty + edge length, Utility: Info gain from neighbors. The one-step estimated reward is computed as a function of the information gain and cost associated with an action:
\begin{align}
...
\begin{align}
    H_p(v^l) = -p_{cov}(v^l) \log p_{cov}(v^l) - \left(1-p_{cov}(v^l)) \log (1-p_{cov}(v^l)\right)
\end{align}
and the expected entropy after measuring is $E[H_{p}(\textbf{m}_i)]$. 

Then the expected information gain when sensing a grid cell $\textbf{m}_i$ is: 
\begin{align}
    IG(v^l; z) &= H_p(v^l) - E[H_{p}(v^l; z)] \\
    IG(V; z) &= \sum_{v^l \in V} IG(v^l; z)
\end{align}
...
Reward function as a weighted sum of information gain and traversal cost

\begin{align}
    \hat{R}(s, a) = c_{IG} IG(V; z) - c_{T} Cost()
\end{align}
where $z = \hat{O}(s, a)$
