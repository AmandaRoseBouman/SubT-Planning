...
\gautam{Name suggestion: PLGRM: Planning at Local and Global level with Road Maps. Pronounced pilgrim}
...
In \cite{dang2019explore}, the lower level module is extended to a frontier selection algorithm considering the information gain along the path.
\citet{Lauri2016planning} replaced the lower level module by a POMDP-based planner, so that the local coverage performance can be improved 


% Hierarchical (online-offline) POMDP solver, for navigation, not for coverage planning
% \cite{kim2019bi}

% Hierarchical Monte-Carlo planning \cite{vien2015hierarchical}
...
\ph{[WIP] Coverage in unknown space}
Even worse, ...
...
\subsubsection{Algorithm: NAME?? PLGRM: Planning at Local and Global level with Road Maps}
...
Here, Qg = robot q and $\dot{q]}$. Gg = Simple bidirectional graph.
Generative model $\mathcal{G}$ is 

\subsubsection{Local Level}

\gautam{PUT SOME DESCRIPTION IN TEXT ABOUT THE ALGORITHM/FRAMEWORK}
...
\begin{algorithmic}
\REQUIRE State $s^g=(G^g, Q^g)$
\REQUIRE World configuration $W$
\REPEAT
    \STATE \textbf{Global level planner}
    \item  GLP -- obtain world map
    \item Estimate high-level generative model $\hat{\mathcal{G}}^g$ and create high-level POMDP
    \item $\theta^l_t \gets \textsc{POMCP-GLP}(Q^g, G^g, \hat{\mathcal{G}^g})$
    
    \STATE \textbf{Low level planner}
    \item LLP -- obtain local map
    \item Estimate low-level generative model $\hat{\mathcal{G}}^l$ and create low-level POMDP
    % \item $\hat{\pi}^{*l} \gets \textsc{POMCP-LLP}(Q^l,G^l,\hat{\mathcal{G}}^l ; \theta^l_t)$ \#learn value function at low level
    \item $a_{t:T}^{*l} \gets \textsc{POMCP-LLP}(Q^l,G^l,\hat{\mathcal{G}}^l ; \theta^l_t)$ \#learn value function and get best actions
    
    \STATE \textbf{Executive}
    \item Execute actions $a_{t:T}^{*l}$
    \item Obtain observation $O$ from sensors
    \item Update robot state $Q$, world configuration $W$
\UNTIL timeout
\end{algorithmic}
\end{algorithm}
