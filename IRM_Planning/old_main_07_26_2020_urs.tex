...
\newcommand{\ph}[1]{} % paragraph header
...
\subsection{Background: Basic POMDP Formulation}
We briefly introduce the POMDP formulation first.
...
\subsection{Coverage Planning Problem in Unknown Environments}
...
% \subsubsection{From Coverage Problems to POMDPs}
\subsubsection{Coverage Problem Description}
...
Since ECSP problems don't have access to the true $V$, so its optimization objective is to maximize the number of visited nodes and to minimize the traversal distance at the same time.
With consideration of motion and sensing uncertainty of the agent, then it can now be extended to a general POMDP problem but with a special reward function for coverage.

% By predicting how future sensor information and vehicle control will affect its world belief, the agent can make long-horizon, near-optimal decisions that are robust to environment uncertainty.

% under motion and sensing uncertainty. 

% In a receding horizon fashion, we can find paths that cover the known space very efficiently, and we design the reward function to balance complete coverage and exploration.
...
% Although the environment is known a priori 
...
\subsubsection{POMDP formulation of Coverage Problems}
...
% BCSP->POMDP for coverage in unknown env
...
\subsubsection{Challenges in Coverage Problems} 

\ph{Challenges with BCSP} 
...
\ph{Traditional Map Representation} Consider a traditional mapping formulation for coverage. The belief state in this case is $b = p(M, x)$ where $M$ is the map state and $x$ is the robot state. We model $M$ as an occupancy map composed of grid cells  $m = \{\textbf{m}_i\}$ where $\mathbf{m}_i$ is a random variable representing some state (i.e. occupancy, coverage) of the $i$-th cell. In order to compute the posterior probability $p(M | z_{1:t}, x_{1:t})$ of the whole map $M$, given the robot's measurements $z_{1:t}$ and trajectory $x_{1:t}$, the cell states are traditionally approximated as binary with full independence between them \cite{TBF05,elfes1990stochastic}. %6
...
% \section{PLGRIM: Planning at Local-Global levels with Robust Information roadMaps}
% \section{PLGRIM Framework}
\section{PLGRIM for Large-scale Coverage Planning}\label{sec:plgrim}
...
In this work, we propose Planning at Local-Global levels with Robust Information roadMaps (PLGRIM), a novel framework of hierarchical coverage planner with information roadmaps in a POMDP setting.
...
\subsection{Coverage Planner with Information Roadmap} 
...
%
We present a \textit{coverage planner} with an abstract environment representation which is called information roadmap.
...
More detailed instantiation of the proposed framework is provided in Appendix~\ref{sec:appendix}.
