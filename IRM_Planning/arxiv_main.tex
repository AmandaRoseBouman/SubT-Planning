...
Value learning over belief spaces suffer from computational challenges, especially for long temporal horizons and large spatial environments for exploration. % \acomm{Why are spatial envs and long horizons high dimensional? (low priority addition)}
...
% We maintain two representations of the world state: $W = (W_{r}, W_{c})$, where $W_{r}$ and $W_{c}$ describe the risk and the coverage states of the world, respectively.
...
% where $W_{r}$ and $W_{c}$ describe the risk and the coverage states of the world, respectively.


% maintain two representations of the world
% occupancy --> risk world state
...
% \ph{Risk World} The risk world state $W_{r}$ encodes the traversability risk over the world. Fig. ?? illsustrates an exmaple ... where black colors demonte nontraversa areas such as large boulders, slopes, steps, or ... and white part reptesntats the surfaces that robot can traverse... and heatmap cpatures the full spectrum... $W_{r}$ is encoded on dense graph wiht quantized risk values (see Section ?? for details).
...
% A dense risk map \textit{Riskmap} (see Fig.~\ref{fig:IRMs}) pr
% Fig. ?? colored by risk value 
% illsustrates an exmaple ... where black colors demonte nontraversa areas such as large boulders, slopes, steps, or ... and white part reptesntats the surfaces that robot can traverse... and heatmap cpatures the full spectrum... $W_{r}$ is encoded on dense graph wiht quantized risk values (see Section ?? for details).
% \ph{Coverage Map} 
...
% sensor measurements gathered along the path taken by the robot. 
% new sensor The robot \textit{gains information} as it accumulates new sensor measurements of the world. 
% Given a known environment represented by an abstract graph structure $W = (V, E)$, with free and occupied nodes $V_{free}\cup V_{occ} = V$, the traditional coverage problem aims to find a sequence of nodes and edge traversals that pass through all free nodes $V_{free} \subseteq V$. 
...
% The occupancy and coverage states inform the action cost $C$ and information gain $I$, respectively, and form the basis of the coverage reward function:
...
We detail the challenges associated with solving the coverage planning problem, Eq.~(\ref{eq:receding_objective_function}), in an unknown, large ($>\!\!1$~km), and rugged-terrain environment over long-time scales ($>\!\!1$~hour). % using a POMDP-RHP formulation.
...
% We broadly identify these challenges as computational complexity---in both space and time---for each planning episode and receding-horizon policy reconciliation over consecutive planning episodes.
...
%     \item \label{en:idea3} We propose use of efficient POMDP solvers at each hierarchical level to reason over a longer temporal horizon within the given replanning time. 
%   \end{enumerate}
...
%   a Receding Horizon Planning (RHP) scheme that is resilient to sudden belief updates.    
...
% In the unknown environment exploration domain, the ground-truth world state $W^*$ is not accessible at runtime.
...
\end{align}
% and frontier nodes capture the \textit{uncovered free} space:
% \begin{align}
% %   &N^g_b \subset \{\hat{w}_i | P(\hat{w}_{i,\neg occ} \wedge \hat{w}_{i,cov}) > \bar{\psi}_b\}, \\
%   N^g_f \subset \{\hat{w}_i | P(\hat{w}_{i,\neg occ} \wedge \hat{w}_{i,\neg cov}) > \bar{\psi}_f\}. \notag
%   \label{eq:coverage_belief}
...
% \begin{align}
%   &\exists \, \hat{w}^g_j \in \mathcal{N}_{G^g}(\hat{w}_i) such that the traversal risk \rho_{ij} \in [0, 1] < \bar{\psi}_e for \hat{w}^g_i and \hat{w}^g_j
%   \label{eq:existance_of_path}
% \end{align}
...
% \st{i.e., $\exists \, \hat{w}^g_j \in \mathcal{N}_{G^g}(\hat{w}_i)$ such that the traversal risk $\rho_{ij} \in [0, 1] < \bar{\psi}_e$ for $\hat{w}^g_i$ and $\hat{w}^g_j$, where $\mathcal{N}_{G^g}(\cdot)$ is a set of nearby nodes in $G^g$, and $\bar{\psi}_e$ is some threshold.}
% An additional condition that must be satisfied for a candidate node $\hat{w}_i$ to be classified as either a breadcrumb or frontier node is that there should be a traversable path to nearby existing nodes in $G^g$, i.e., $\exists \hat{w}^g_j \in \mathcal{N}_{G^g}(\hat{w}_i)$ such that the traversal risk $\rho_{ij} \in [0, 1] < \bar{\psi}_e$ for $\hat{w}_i$ and $\hat{w}^g_j$, where $\mathcal{N}_{G^g}(\cdot)$ is a set of nearby nodes in $G^g$ and $\bar{\psi}_e$ is some threshold.
%
...
\centering
    \begin{tikzpicture}
	   ...
	   % 	\node [font=\fontsize{4pt}{0}, above left,align=right,black] at (0.26,-0.01) {$(W_{4a},Q_{4a})$};
	    ...
	    \end{scope}
	\end{tikzpicture}	
  \caption{Illustrative example of coverage path planning on the Local IRM with Monte-Carlo Tree Search. Robot field-of-view is represented by blue circle. The state contains the world coverage state, $W$, and the robot state, $Q$. Macro actions (6 steps on Local IRM) associated with two tree branches (paths \textit{A} and \textit{B}) are shown. Note that the final coverage world state in each branch are identical. Path \textit{A} is evaluated to be more rewarding than \textit{B} since fewer actions were required to cover the same area.}
...
% % 
% % Specifically, a target frontier node in the Global IRM, $\hat{w}^g_f \in N^g$, can be extracted from the global-level control $a^g$.
% %
% If the target frontier $\hat{w}^g_f$ is within the Local IRM range, i.e., $\hat{w}^g_f \in \hat{W}^\ell$, then the first mode is activated, and LCP performs the normal information-gathering coverage optimization, as described in Eq.~(\ref{eq:llp_optimization}).
...
\centering
    \begin{tikzpicture}
	    ...
	    \begin{scope}[x={(image.south east)},y={(image.north west)}]
	    	...
	    	% Annotations [Letters]
	   % 	\node [above left,align=right,black] at (0.07,0.975) {(a)};
    	    \node [above left,align=right,black] at (0.42,0.99) {(a) Simulated Subway};
	   % 	\node [above left,align=right,black] at (0.07,0.67) {(b)};
...
\centering
    \begin{tikzpicture}
	    \node[anchor=south west,inner sep=0] (image) at (0,0) {\includegraphics[width=1\textwidth]{figures/point_cloud_lava_tube_labels_v6.png}};
	    \begin{scope}[x={(image.south east)},y={(image.north west)}]
	    
	    	% Annotations [Letters]
	    	\node [above left,align=right,black] at (0.575,0.455) {H};
	    	\node [above left,align=right,black] at (0.7,0.59) {A-C};
	    	\node [above left,align=right,black] at (0.765,0.1) {F-G};
	    	\node [above left,align=right,black] at (0.815,0.325) {D};
	    	\node [above left,align=right,black] at (0.895,0.855) {E};
	    	
	    	\node [above left,align=right,black] at (0.097,0.17) {XX m};
...
\centering
    \begin{tikzpicture}
	    ...
	    \end{scope}
	\end{tikzpicture}	
\caption{LCP plans a looping path that fully covers area represented by Local IRM (snapshot A). Path is updated to extend towards large uncovered swath while maintaining consistency with the prior path. Riskmap update reveals that path has entered a hazardous area---wall of lava tube (snapshot B). As a demonstration of LCP's resilience, path shifts away from the hazardous area and robot is re-directed towards the center of the tube (snapshot C). One minute later, robot encounters a fork in the cave system. LCP's path curves slightly toward fork apex before entering a larger, less-risky channel (snapshot D). } \label{fig:mlp_hardware_tests} 
\end{figure*}
...
\centering
    \begin{tikzpicture}
	    ...
	    \end{scope}
	\end{tikzpicture}	
\caption{GCP plans path (blue) along the Global IRM to a target frontier after the local area is fully covered (snapshot E). The robot explores the area around the frontier (snapshot F), and then explores a neighboring frontier at the opening of a narrow channel. LCP plans a path (green) into the channel (snapshot G). Later, after all local area has been explored, GCP guides the robot back towards the mouth of cave (snapshot H).} \label{fig:glp_hardware_tests} 
\end{figure*}
