...
\title{
% PLGRIM: Hierarchical Information Roadmap Planning for Large-scale Coverage in Unknown Environments
% PLGRIM: Hierarchical POMDP Planning for Large-scale Coverage in Unknown Environments
OLD OLD OLD OLD OLD, switch to main.tex!!!!\\
...
\begin{abstract}
In order for a mobile robot to achieve a useful level of autonomy in previously unexplored environments, it must take into account uncertainties in localization, sensor measurements, and unknown hazards, as well as the effect of robot's own actions on these uncertainties. While there are many general approaches for planning under uncertainty, many of these methods do not scale well to problems with very large spatial and temporal horizons required in typical real-world missions. %An effective planner must be computationally efficient and handle time constraints without severely impacting the optimality of the solution.
In this work, we propose a scalable planner under uncertainty: PLGRIM (Planning at Local-Global levels with Robust Information roadMaps), that bridges the gap between \textit{(i)} local, high-fidelity information such as traversability, and \textit{(ii)} global, macro-level reward seeking behaviors.
...
PLGRIM pushes the boundaries  of  the  state-of-art  in enabling physical robots to accomplish real-world complex missions in relevant scenarios. As concrete example, we implement PLGRIM on physical systems (Boston Dynamics  Spot  robot) and demonstrate its performance as the planning core of the NeBula 2.0 autonomy framework. NeBula 1.0 (prior to PLGRIM integration) framework and the adopted hardware in this demonstration have contributed to winning first place in the 2020 DARPA Subterranean Challenge, Urban Circuit. \ali{not good at this point. i will udpate the new parts.}
...
\keywords{Coverage under uncertainty, Hierarchical planning, POMDP} 
...
A coverage planning problem in an unknown space can be considered an active learning problem under uncertainty where the objective is to find the best action sequence to maximize the accumulated reward over time.  The agent must accumulate data to incrementally build a model of its environment, and must understand the effect of its actions on the quality and quantity of data it collects.
...
Since the agent's future actions affect the its belief of the world and robot state, we cast this coverage problem as a Partially Observable Markov Decision Process (POMDP) problem. 
A POMDP is a principled formalization of sequential decision making process under motion and sensing uncertainty.
It employs the underlying intrinsic model of the sequential action-observation process under uncertainty, so that it can (asymptotically) converge to the optimal solution in a more sample-efficient manner than model-free learning approaches.
...
\begin{figure}[ht!]
  \centering
  \subfloat[[WIP] System diagram]{%
    \includegraphics[width=.6\textwidth]{figures/SystemOverview.png}
  }
%   \subfloat[Visualization of hierarchical IRMs]{%
%   \subfloat[Hierarchical IRMs]{%
  \subfloat[Global/local IRMs]{%
    \includegraphics[width=.3\textwidth]{IRM_Planning/figures/graph-lattice-map.pdf}
  }
...
In general, however, POMDPs suffer from the curse of dimensionality and are difficult to solve.  There are many algorithms for finding solutions POMDPs with varying degrees of efficiency and accuracy, and more continue to be developed today \cite{silver2010monte,somani2013despot,bonet1998learning,kim2019pomhdp}.  However, because in our setting we aim to tackle problems which have large time horizons (> 1 hour), large spatial extents (> 10 km$^2$), and high dimensional belief states (including beliefs on the state of the environment), determining a solution for the robot's behavior over the full belief becomes intractable.  
%

\ph{Contributions}
Therefore, we introduce several key spatial and temporal discretizations which restrict the behavior of the robot to tractable search space.  This decomposition allows us to approximately solve the optimization problem in a computationally tractable manner.  We discretize the belief space into a robot and task-relevant graph structure enriched with future map estimates, which reduces our search space for good policies, which we call an Information Roadmap (IRM) (Figure \ref{fig:firstPage}).  We further decompose the problem into a global IRM which spans the entirety of the known environment, and a local IRM which is robot-centric and of fixed size.  We then solve the entire hierarchical POMDP problem in a receding horizon fashion, online and in real time.

% We then create multi-levelmulti-resolution map representation enriched with future map estimates. 
% We evaluate the expected perceptual information gain and traversability risk on a local dense map representation. This map is then discretized to achieve a global, computationally-efficient map representation, spanning the entirety of the known environment, and enriched with the locally gathered map estimates. High level actions are planned on the global map, and then converted to low level actions on dense map.    

\ph{Outline}
Section~\ref{sec:related_work} presents related work, and Section~\ref{sec:ECSPasPOMDP} formally describes the problem of interest.
Section~\ref{sec:plgrim} presents PLGRIM, our IRM-based hierarchical coverage planning framework,  Section~\ref{sec:exp_results} shows experimental results in simulation and hardware validating our method, and
...
Frontier-based exploration is a widely used approach for autonomous exploration \cite{yamauchi1997frontier,tao2007motion,keidar2012robot,heng2015efficient,gonzalez2002navigation,grabowski2003autonomous}.
...
% For example, the Next-Best-View algorithm considers the range sensor footprint after one future step and evaluates the next best action \cite{}.  This approach is \textit{complete} but can be significantly suboptimal in time.
%
By continuing exploration until exhausting all remaining frontiers, frontier-based approaches can guarantee \textit{completeness} of the coverage of reachable spaces.  However, because they follow a one-step look-ahead greedy policy by selecting the best frontier upfront, these algorithms are subject to local minima and are suboptimal in time.
...
Model-free reinforcement learning (RL) has been applied to coverage and exploration problems \cite{pathak_icm, rnd,burda2018study,ECR2018}.  In this setting the typical approach is to find a policy which maps sensor data to actions, to maximize the reward.  However, this approach is less useful for long-range, large-scale autonomous robot missions, in which safety-critical constraints and the expense of collecting data play a large role.

% Reinforcement Learning (RL) has been applied to exploration by using rewards internal to the agent \cite{pathak_icm, rnd}; the more difference between your predicted and actual visual input, the more the internal curiosity reward of the agent. This, however, fails when the agent can take actions that don't explore parts of the world but still change visual input, such as by changing a TV channel \cite{burda2018study}. To focus the exploration on movement in an environment, \citet{ECR2018} proposed agents that mark parts of the map as far away, as described by the environment.
% %
...
%
POMDP-based approaches generate a non-myopic policy by considering long-horizon action sequences, interactively learning the value function of each, and returning the best action sequence that maximizes the accumulated rewards.
% While there is an intrinsic challenge of high computational complexity, this approach has its own strength in its simultaneous handling of both actions and observations for sequential decision making.

To reduce the POMDP problem complexity in coverage and exploration problems, several techniques have been explored.  \cite{indelman2015planning} and \cite{martinez2009bayesian} employ a direct policy search scheme with a Gaussian belief assumption.  \citet{Lauri2016planning} extended this to non-Gaussian beliefs based on the POMCP (Partially Observable Monte-Carlo Planning) algorithm that uses a Monte-Carlo Tree Search \cite{silver2010monte}.
% They also It also used the direct policy search approach to generate possible action sequences with a Gaussian kernel.
Nevertheless, even with such techniques for approximation the POMDP-based approach does not scale well due to the curse of history and dimensionality \cite{Pineau03}.
...
% Sec4 Problem Definition

% CONTENTS
% - assumptions: 
% - objective: coverage problem optimization problem
% - challenges: 

% STRUCTURE
% - problem domain
%     . coverage problem definition: (V, E) + FIND
%     . covering salesman problem: (V, E) + OPT
%     . pomdp (CSP with motion/sensing uncertainty)
% - POMDP formulation (for coverage!)
%     : b, r(), ... + OPT(info\_gain)
%     ? how much into the details?
% - POMDP challenges for coverage in unknown env
%     : computational complexity
%     : no access to ground truth gen model


% Sec5 PLGRIM

% CONTENTS
% - our solution for POMDP challenges for coverage in unknown env
%     : PILGRIM framework
%         . hierarchical structure
%         . belief representation: IRM/Lattice
%         . policy solver: POMDP with est.gen() + belief representation
%     : contribution
%         . hierarchical framework for scalability
%         . IRM/Lattice construction for information-rich abstract ...
%             , traverbility, occupancy, coverage, ..., artifact signal, ...
%             -> compact/hier-level-agnostic representation but also informative (extendable?)
%         % . policy solver: POMDP with est.gen()

% STRUCTURE
% - hierarchical planning
%     : belief representation: graph
%         . encode multi-dimensional features
%     : POMDP solver connection
%         . high-level policy is low-level hyper-parameter
%     : unified policy formulation/connection
%         . formulation
...
% what cannot be generalized as abstract POMDP format
% : what is specific to 
% - 
% - transition model
...
% - NOTATION TABLE?! (common components)
%     : graph-level
%     : lattice-level
...
%     : belief representation: graph
%         . encode multi-dimensional features
%         . explain IRM and Lattice
%     : POMDP formulation at each level
%         . common (sec4)
%         . specific (sec5)
%     : POMDP solver connection
%         . high-level policy is low-level hyper-parameter
...
We first briefly introduce the coverage problem as a specific traveling salesman problem, and show how we formalize this problem as a POMDP.  We then discuss why a naive solution approach to this problem can easily become intractable.  

\ph{Covering Salesman Problem (CSP)} Given a known environment represented by an abstract graph structure $W = (V, E)$, with free and occupied nodes $V_{free}\cup V_{occ} = V$, the traditional coverage problem aims to find sequence of nodes and edge traversals that pass through all free nodes $V_{free} \subseteq V$.  By incorporating a sensor field-of-view $F:V\rightarrow \mathcal{P}(V)$, which maps each node to a subset of "viewable" nodes, and adding the objective of minimizing the traveled distance, the coverage problem can be seen as a specific type of traveling salesman problem -- the \emph{covering salesman problem} (CSP).  In the CSP the objective is to determine the minimum length path through a subset of nodes $\{v_i\}_i \subseteq V_{free}$ such that every free node is within the accumulated sensor field-of-view: $V_{free} \subseteq \cup_i F(v_i)$.
% \ph{CSP Extension} In a CSP, the environment is only partially revealed to the agent due to its limited field-of-view $F$. Given a known environment, the CSP path planning objective is to find the minimum sequence of node transitions which leads to zero spatial uncertainty. 
...
\ph{State} We define the system (robot-world) state as a 2-tuple $s = (W, Q) \in \mathbb{W}\times\mathbb{Q} =  \mathbb{S}$ consisting of the current continuous world representation $W$ and $Q$ representing the robot state. The world can be further decomposed as $W = (W_{occ}, W_{cov})$ where $W_{occ}$ describes the geometry of the world and $W_{cov}$ encodes which regions of the world have been observed by the robot.  For example, $W_{occ}$ can be a mapping from pose to the set $\{0,1\}$, where $W_{occ}(s) = 0$ if the location is free and $W_{occ}(s) = 1$ if the location is occupied (and similarly for $W_{cov}$).
...
% \ph{Belief State} After the agent executes action $a_{t-1}$ and receives observation $z_t$, the belief state can be updated according to Bayes algorithm:
% \begin{align}
%   b_t(s') &= \tau (b_{t-1}, a_{t-1}, z_t) \\
% %   &= P(s' | a_{t-1}, z_t, b_{t-1}) \\
% %   &= \eta \, P(z_t | s', a_{t-1}, b_{t-1}) P(s' | a_{t-1}, b_{t-1}) \\
...
% We formulate the exploration problem under motion and sensing uncertainty as a POMDP problem. We aim to find the optimal policy $\pi^*$ that minimizes spatial uncertainty by solving the following optimization problem:
% % minimizes spatial uncertainty by maximizes a reward consisting of information gain and traversal cost by solving the following optimization problem:
% \begin{align}
%   \pi^*(b) &= \arg\max_\pi \, \mathbb{E} \left[ \sum_{t=0}^{L} \gamma^t r(b_t, \pi(b_t)) \right]
%   \label{eq:optimal_policy}
% \end{align}
% where the one-step coverage reward $r(b_t, \pi(b_t))$ is defined as follows:
...
\ph{Observation} Upon taking an action $a$ in state $s$, the robot receives a full observation of the robot state $Q$ and a partial observation of the world state $W$.  Given a robot state $Q$, the robot observes only areas of the world within the field-of-view of its sensors. Thus, we define the observation $z = w_{cov} \subset W_{cov}$ as the coverage state of the world within a sensor-constrained neighborhood of the robot. The estimated observation model is accordingly defined as:
\begin{align}
%   \hat{O}(s, a, z; \, W_{occ,0}): \, \mathbb{Q} \times \mathbb{W} \xrightarrow{\text{sensor}} \mathbb{Z}
   \hat{O}(s, a, z; \, W_{occ,0}): \, \mathbb{Q} \times \mathbb{W} \xrightarrow{\text{sensor}} \mathbb{Z}
\end{align}
where $W_{occ,0}$ represents the world geometry prior based on the robot's sensing of the real world. \todo{Clarify this} This geometry prior is used to estimate neighborhood coverage $p(Q_{cov})$ using ray-casting. While the world geometry $W_{occ,0}$ is known a priori, only by achieving a robot state $Q$ will the neighboring coverage status of the world be revealed.   
...
% \subsection{Remove this header: Intractability of Traditional Approaches}

% \subsection{Proposed POMDP is Intractable with Traditional Representation}

% \ph{Traditional Map Representation} The traditional belief state for this coverage POMDP is to restrict the world state $W$ to a map representation $M$ and the robot state $Q$ to a continuous vector representing the robot's position $x$, with $b = p(M, x)$. Map $M$ is traditionally modeled as an occupancy map composed of grid cells  $m = \{\textbf{m}_i\}$ where $\mathbf{m}_i$ is a random variable representing some state (i.e. occupancy, coverage) of the $i$-th cell. In order to compute the posterior probability $p(M | z_{1:t}, x_{1:t})$ of the whole map $M$, given the robot's measurements $z_{1:t}$ and trajectory $x_{1:t}$, the cell states are traditionally approximated as binary with full independence between them \cite{TBF05,elfes1990stochastic}. %6
...
% find a trajectory/path
% s.t. (hard constraints): visit all nodes


% Coverage optimization problem (covering salesman problem in known environment)

% minimize (total path length)
% s.t. (visit all nodes)


% Coverage optimization problem (covering salesman problem in unknown environment)

% % minimize (total path length)
% % s.t. (visit all nodes)
% minimize (total path length) - (total path length to visit as many as nodes)


% Covering salesman problem in unknown environment problem

% maximize (information gain) - (total path length to visit as many as nodes) 



% CSP -> exploratory CSP
% -receding horizon
% -discount factor (optional)

% exploratory CSP -> POMDP for coverage in unknown env
% -sensing/motion uncertainty


% CSP -> POMDP for coverage in known env
% POMDP for c. k -> POMDP for c. unknown


% coverage in unknown env -> active learning
% coverage in unknown env -> pomdp



% coverage in unknown env
% maximum mapping (info gathering) + minimum action cost



% \subsection{POMDP Challenges for Coverage in Unknown Environment}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \section{PLGRIM: Hierarchical Coverage Planning}
...
W - function: pose -> cov (t/f), occ (t/f)
v \in V (discrete poses)
W_cov(v) -> t/f, W_{occ}(v) -> t/f
p(W_cov(v) == t)

??? W is for state?
p(W) is probability (belief)


graph-level

p_occ(v \in V^g) = p(W_{occ}^g(v^g))
v\in V, V is set
...
p(\bar{W}_occ) = costmap module(sensor, ...)

Conversion from p(\bar{W}_occ) to p(W_{occ}^g):
average over region around node...  approximate occupancy.
same for local p(\bar{W}_occ) to p(W_{occ}^l). 

pose_graph = p(pos_{0:t})
maximum likelihood estimation
W_cov^g = compute_coverage(V, p(pose_graph))

b^g = p(W_{occ}^g, Q^g) = p(W_{occ}^g) x p(Q^g) = \prod_V [ p(W_occ^g(v)) ] x p(Q^g)
where Q^g \in V^g
...
% $P(W)=\prod_s P(W_{s})$  Associate graph to each little piece $s$.


% $b=p(W,Q)=p(W,Q)=\prod_s P(W_{s},Q_s)=\prod_s P(W_{s}(G),Q_s(G))=\prod_n P(W_{s}(G_n),Q_s(G_n))$

% For each layer (graph/lattice $\{g/l\}$, omitting superscripts for now) we construct a graph $G=(V,E)$ with nodes $V$ and edges $E$.  

% Each node $v \in V \subset \mathbb{S}$ has attached to it a feature vector containing the probability $p(W_{occ}(v))$ that the (todo:  node?  no.. $W_{occ}$) is occupied and the probability $p(W_{cov}(v))$ that the node has been sensed by the robot. 

% Likewise, each edge $e_{ij} \in E$ has attached to it a feature vector containing the probability of traversal between the connected nodes $v_i$ and $v_j$.  
...
% $b^g=p(W^g;v^g)$ cleaner.

% $b^g=p(Q^g;v^g)$.

% $b=p(W,Q)=\prod_{v^g\in V} P(W,Q; v^g)$


\ph{POMDP Specifics} For each planning problem in the hierarchy we define a sub-POMDP problem over the graph representation.  The details of these problems differ in terms of the definitions for transition models, observations, and rewards.  We detail the particular design choices used in this work in the supplementary material, and give the general framework here.
...
% where
% $\pi^{g*}(b) &= \arg\max_\pi \, \mathbb{E} \left[ \sum_{t=0}^{L} \gamma^t r^g(b^g_t, \pi^g(b^g_t)) \right]$ and 
...
% \caption{PLGRIM: Hierarchical Coverage Planner with Information Roadmaps}
...
%     \item  Obtain world information roadmap
...
%     \item Obtain local information roadmap
...
% baseline too good (but some stochasticity)

% next-best-view baseline

% NBV/RandomWalk

% DEEPER PLANNING
% - optimality (open-space)
%  : 1x size station (vs.)
%  : 2x bigger station (vs.)
% 3 runs * 3 baselines * 2 envs

% HIERARCHICAL STRUCTURE
% - scalability (GLP/MLP w/o baseline)
%  ? run
% - scalability (start random + MLP-only until it gets stuck) in cave.world
%  : MLP/GLP
%  : NBV/RandomWalk

% % MLP-only/recovery_behavior(random_walk/drive_forward)/global_frontier vs. MLP/GLP
% MLP-random_walk_to_node vs. MLP/GLP
% MLP-random_walk_locally vs. MLP/GLP

% MLP gets stuck
% MLP set a target to forward/backward
%     1) feasible target
% MS executes..

% MLP: get stuck
%     (sample a breadnode; Dijkstra from the robot->random reachable path)
% MS: sequence path with 1m res? single point inside the map?
% MS->MLP: once it reaches the target, call MLP
% if MLP return nothing, MS->GLP

% MLP IMPROVEMENTS
% : faster planning
%   . faster

% MLP/GLP IMPROVEMENTS
...
\ph{Architecture} Algorithm~\ref{alg:hierarchicalPlanner} outlines the overall PLGRIM framework for hierarchical coverage planning.  (See also Figure \ref{fig:firstPage}).
% As explained in Section~\ref{sec:hierarchical_policy}, the global planner sends waypoints(?) $\theta_{t:T}^l$ to the local planner, which sends its output to the executive. 
% This architecture is displayed in Figure \ref{fig:system_overview}. 
At each timestep (or at given replanning frequencies), both the graph-level and lattice-level planners employ a POMDP solver to find a good policy $\pi^g$, $\pi^l$.  To find these policies we employ an efficient Monte-Carlo Tree Search method for large-scale POMDPs known as POMCP \cite{silver2010monte}.  This method employs a generative model $\mathcal{G}$  (i.e., a black box simulator of the POMDP) in order to eliminate the need for explicit probabilistic representations that account for motion and sensing uncertainty. During each simulation, the initial state is sampled from the initial belief state, and the generative model provides a sample of the successor state $s'$, observation $z$, and reward $r$ based on the dynamics of the environment. However, the true state of the world is unknown.  Thus, an alternative generative model is employed which uses a posterior distribution of the state, $W_0$, which is based on sensor data \todo{Need a little more detail here.  what is $W_0$, $\mathcal{G}$ exactly?}:
\begin{align}
...
% 
% \ph{Efficient POMCP Solver for ECSP-POMDPs} 
% introduced POMCP solvers that use Monte-Carlo Tree Search (MCTS) in the belief space to plan in partially observable environments such as our ECSP-POMDP problem defined in Section~\ref{sec:ECSPasPOMDP}.

% \begin{figure}
%     \centering
%     \includegraphics[width=0.7\columnwidth]{IRM_Planning/figures/POMCPTree.png}
%     \caption{A sample tree from POMCP applied at the lattice-level}
%     \label{fig:my_label}
% \end{figure}
...
% \begin{figure}[ht!]
%   \includegraphics[width=.6\textwidth,trim=3cm 0 4cm 0, clip]{figures/dist_traveled.pdf}
%   \centering
